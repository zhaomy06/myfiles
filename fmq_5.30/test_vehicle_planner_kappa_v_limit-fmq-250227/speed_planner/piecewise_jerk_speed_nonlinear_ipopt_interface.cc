...
bool PiecewiseJerkSpeedNonlinearIpoptInterface::get_bounds_info(
  ...
  //s[target time]-tar_s
  g_l[offset+num_of_points_-1]=0.0;
  g_u[offset+num_of_points_-1]=0.0;
  offset+=1;
...
bool PiecewiseJerkSpeedNonlinearIpoptInterface::eval_g(int n, const double *x,
  ...
  g[coffset_velocity+num_of_points_-1]=x[int(target_time/delta_t_)]-target_s;

  offset += 3 * (num_of_points_ - 1)+1;
...
bool PiecewiseJerkSpeedNonlinearIpoptInterface::eval_jac_g(
  ...
  if (values == nullptr) {
    ...
    iRow[non_zero_index] = constraint_index;
    jCol[non_zero_index] = int(target_time/delta_t_);
    ++non_zero_index;
    ++constraint_index;
...
void PiecewiseJerkSpeedNonlinearIpoptInterface::finalize_solution(
  ...
  if (use_soft_safety_bound_) {
    ...
    std::cout << "lower soft s boundary average intrustion is ["
           ...
           << "].";
    std::cout << "upper soft s boundary average intrustion is ["
...
void PiecewiseJerkSpeedNonlinearIpoptInterface::set_tar_t_and_s(const double tar_t,const double tar_s){
  target_time=tar_t;
  target_s=tar_s;
}
}  // namespace planning
